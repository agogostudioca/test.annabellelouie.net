...
function loadHtml5Gallery(u) {
	...
	v.fn.touchSwipe = function(b) {
		...
		return this.each(function() {
			...
			function d(a) {
				if (0 < l || 0 < g)
					100 < Math.abs(l - f) && (l > f ? e.swipeRight && e.swipeRight.call() : e.swipeLeft && e.swipeLeft.call()), 100 < Math.abs(g - h) && (g > h ? e.swipeBottom && e.swipeBottom.call() : e.swipeTop && e.swipeTop.call());
				j(a)
			}
			function j() {
				g = l = h = f = -1
	...
	var s = n.event,
		...
		k = x.drag = {
			...
			delegate: function(b) {
				var e = [],
					a,
					c = n.data(this, "events") || {};
				n.each(c.live || [], function(d, c) {
					if (0 === c.preType.indexOf("drag") && (a = n(b.target).closest(c.selector, b.currentTarget)[0]))
						s.add(a, c.origType + "." + k.livekey, c.origHandler, c.data), 0 > n.inArray(a, e) && e.push(a)
				});
			...
			hijack: function(b, e, a, c, d) {
				if (a) {
					...
					a.results = [];
					do if ((t = a.interactions[g]) && !("dragend" !== e && t.cancelled))
						q = k.properties(b, a, t), t.results = [], n(d || t[h] || a.droppable).each(function(d, c) {
							q.target = c;
	...
	m.fn.addHTML5VideoControls = function(b, e, a) {
		...
		var B = m(".html5boxTimeCurrent",
			w),
			O = m(".html5boxTimeTotal", w),
			C = m(".html5boxSeeker", w),
			r = m(".html5boxSeekerPlay", w),
			S = m(".html5boxSeekerBuffer", w),
			J = m(".html5boxSeekerHandler", w);
		B.css({
			display: "block",
			position: "relative",
			"float": "left",
		...
		if (!k && (p.get(0).requestFullscreen || p.get(0).webkitRequestFullScreen || p.get(0).mozRequestFullScreen || p.get(0).webkitEnterFullScreen || p.get(0).msRequestFullscreen)) {
			var E = function(a) {
				...
				var b = M.css("background-position") ? M.css("background-position").split(" ")[1] : M.css("background-position-y");
				M.css({
				...
				a ? (m(document).on("mousemove", D), w.css({
					"z-index": 2147483647
				})) : (m(document).off("mousemove", D), w.css({
					"z-index": ""
				}))
			};
			...
			M.css({
				...
				(t = !t) ? (p.get(0).requestFullscreen ? p.get(0).requestFullscreen() :
		...
		if (p.get(0).volume === a / 2 + 0.1) {
			...
			var K = m(".html5boxVolumeButton", w),
				F = m(".html5boxVolumeBar", w),
				y = m(".html5boxVolumeBarBg", w),
				L = m(".html5boxVolumeBarActive", w);
			a.css({
				display: "block",
				position: "relative",
				"float": "right",
			...
			K.css({
				...
				var a = m(this).css("background-position") ? m(this).css("background-position").split(" ")[0] :
				m(this).css("background-position-x");
				m(this).css({
					"background-position": a + " top"
				})
			}).on("click", function() {
		...
		try {
			p.on("play", function() {
				...
				A.hide();
				I.show()
			}), p.on("pause", b), p.on("ended", b), p.on("timeupdate",
			d), p.on("progress", f)
		} catch (X) {}
	};
	var b = jQuery,
		...
		e.prototype = {
			...
			checkMK: function() {
				if (this.options.fv) {
					...
					if (0 > a.text().indexOf(this.options.fvm) || 0 > c.length)
						a.append("<a style='text-decoration:none;' target='_blank' href='" + this.options.freelink + "' ><div style='display:block;visibility:visible;position:absolute;top:10px;left:10px;width:170px;height:18px;line-height:18px;text-align:center;border-radius:5px;-moz-border-radius:5px;-webkit-border-radius:5px;background-color:#fff;color:#333;font-size:12px;font-family:Arial,Helvetica,sans-serif;'>" + this.options.fvm + "</div></a>");
					else if (a = c.find("div"),
					"none" == c.css("display") || "hidden" == c.css("visibility") || 8 > parseInt(c.css("font-size")) || "none" == a.css("display") || "hidden" == a.css("visibility") || 8 > parseInt(a.css("font-size")))
						c.attr({
			...
			initData: function(a) {
				...
				if (this.options.src && 0 < this.options.src.length) {
					...
					this.options.remote && 0 < this.options.remote.length ? (d = this, b.getJSON(this.options.remote, function(b) {
			...
			readTags: function() {
				...
				(b("img.html5galleryimg", this.container).length ? b("img.html5galleryimg", this.container) : b("img", this.container)).each(function() {
					...
					var h = a.options.width,
						...
						A = null,
						D = !1;
					b(this).parent().is("a") && (d = b(this).parent().attr("href"),
					...
					z = b(this).parent().data("youtubeplaylistid"), A = b(this).parent().data("youtubeplaylistmaxresults"), D = b(this).parent().hasClass("html5gallerylightbox"));
					var B = b(this).parent().data("mediatype") ? b(this).parent().data("mediatype") : a.checkType(d);
					a.elemArray.push([a.elemArray.length, c, d, t, q, k, m, e, f, B, h, l, n, s, p, u, g, v, x, w, z, A, D])
				})
			},
			...
			updateTimer: function(a) {
				this.isFullscreen ? (a = Math.max(0,
				Math.floor(b(".html5gallery-fullscreen-elem-wrapper-" + this.id, this.$fullscreen).width() * a)), b(".html5gallery-fullscreen-timer-" + this.id, this.$fullscreen).css({
					width: a + "px"
			...
			createEvents: function() {
				...
				if (!("ontouchstart" in window) || !a.options.disablehovereventontouch)
					b(".html5gallery-box-" + this.id, this.$gallery).mousemove(function() {
						...
						if (a.options.titleoverlay && a.options.titleautohide && (!a.options.hidetitlewhenvideoisplaying || !a.isVideoPlaying))
							b(".html5gallery-title-" + a.id, a.$gallery).show(), clearTimeout(a.hideTitleTimeout),
				...
				if ("vertical" == this.options.skin || "showcase" == this.options.skin)
					b(".html5gallery-car-list-" + this.id, this.$gallery).on("DOMMouseScroll mousewheel wheel", function(c) {
						c.preventDefault();
				...
				if (this.container.parent() && this.container.parent().width()) {
					...
					if (1 == a || this.showingPoster) {
						...
						if (this.isFullscreen) {
							...
							d = "fill" == this.options.resizemode ? Math.max(this.options.width / c, this.options.height /
			...
			createStyleVertical: function() {
				...
				if (!this.options.showsocialmedia || !this.options.showfacebooklike &&
				!this.options.showtwitter && !this.options.showgoogleplus)
			...
			resizeStyleDefault: function() {
				if (this.container.parent() && this.container.parent().width()) {
					...
					if (this.options.showcarousel) {
						...
						this.options.carouselmultirows ? ("samecolumn" == this.options.thumbresponsive ? (this.options.carouselcolumn = this.options.thumbcolumns, this.options.thumbcolumnsresponsive && (a = b(window).width(), a < this.options.thumbsmallsize ?
						this.options.carouselcolumn = this.options.thumbsmallcolumns : a < this.options.thumbmediumsize && (this.options.carouselcolumn = this.options.thumbmediumcolumns)), this.options.thumbwidth = Math.min((this.options.width - this.options.thumbgap * (this.options.carouselcolumn - 1)) / this.options.carouselcolumn), 

								// this.options.thumbheight = this.options.thumbwidth * this.options.thumboriginalheight / this.options.thumboriginalwidth, 

								this.options.thumbimagewidth = this.options.thumbwidth - 2 * this.options.thumbimageborder, this.options.thumbimageheight =
						this.options.thumbheight - 2 * this.options.thumbimageborder, this.options.thumbshowtitle && (this.options.thumbheight += this.options.thumbtitleheight)) : 
							...
							(this.options.thumbcolumnsresponsive && (a = b(window).width(), a < this.options.thumbsmallsize ? (this.options.thumbwidth = this.options.thumbsmallwidth
								...
								) : a < this.options.thumbmediumsize ? (this.options.thumbwidth = this.options.thumbmediumwidth
						...
						), this.options.thumbimagewidth = this.options.thumbwidth - 2 * this.options.thumbimageborder, this.options.thumbimageheight = this.options.thumbheight - 2 * this.options.thumbimageborder, this.options.thumbshowtitle && (this.options.thumbheight += this.options.thumbtitleheight)), this.options.carouselcolumn = Math.floor(this.options.width / (this.options.thumbwidth + this.options.thumbgap)), 1 > this.options.carouselcolumn && (this.options.carouselcolumn =
						1)), this.options.carouselHeight = Math.ceil(this.elemArray.length / this.options.carouselcolumn) * (this.options.thumbheight + this.options.thumbrowgap)) : this.options.thumbcolumnsresponsive && (a = b(window).width(), a < this.options.thumbsmallsize ? (this.options.thumbwidth = this.options.thumbsmallwidth//, 
						// this.options.thumbheight = this.options.thumbsmallheight
						) : a < this.options.thumbmediumsize ? (this.options.thumbwidth = this.options.thumbmediumwidth
						// , this.options.thumbheight = this.options.thumbmediumheight
						) : (this.options.thumbwidth =
					...
					if (1 == a || this.showingPoster) {
						...
						d = this.elemArray[this.curElem][10], c = this.elemArray[this.curElem][11], this.isFullscreen ? (a = this.elemArray[this.curElem][17] ?
			...
			goFullscreen: function() {
				...
				b(".html5gallery-fullscreen-box-" + this.id, this.$fullscreen).hover(function() {
					1 < g.elemArray.length && (b(".html5gallery-fullscreen-next-" + g.id, g.$fullscreen).fadeIn(), b(".html5gallery-fullscreen-prev-" +
					g.id, g.$fullscreen).fadeIn())
				}, function() {
				...
				b(document).on("keyup.html5gallery", function(a) {
					27 == a.keyCode ? g.goNormal() : 39 == a.keyCode ? g.slideRun(-1) : 37 == a.keyCode &&
					g.slideRun(-2)
				})
			},
			...
			slideRun: function(a, c, d, e) {
				...
				if (this.options.showtitle || this.options.lightboxshowtitle || this.options.lightboxshowdescription) {
					...
					if (this.isFullscreen) {
						...
						(h = "", a && (h += "<div class='html5gallery-title-text-" + this.id + "'>" + a + "</div>"), this.options.showdescription && f && (h += "<div class='html5gallery-description-text-" + this.id + "'>" + f + "</div>"), b(".html5gallery-title-" + this.id, this.$gallery).html(h));
					(!this.options.titleoverlay || !this.options.titleautohide) && b(".html5gallery-title-" + this.id, this.$gallery).show()
				}
				a = this.elemArray[this.curElem][9];
				if (!(0 > a)) {
					!this.isFullscreen && c ? "always" == this.options.showimagetoolbox ? ("mouseover" == this.options.imagetoolboxmode ||
					"show" == this.options.imagetoolboxmode) && this.showimagetoolbox(a) : "image" == this.options.showimagetoolbox && 1 != a && this.hideimagetoolbox() : "show" == this.options.imagetoolboxmode ? this.showimagetoolbox(a) : this.hideimagetoolbox();
					this.onChange();
					...
					11 == a ? this.showEmbedVideo(d) : 2 == a ? this.showSWF() : 12 == a && this.showIframe()));
					this.checkMK();
					...
					(!this.options.autoplayvideo && this.elemArray[this.nextElem][12]) && ((new Image).src = this.elemArray[this.nextElem][12]);
					this.curElem == this.elemArray.length - 1 && this.looptimes++;
					var g = this;
					if ((1 == a || this.showingPoster) && !this.isPaused && 1 < this.elemArray.length && (!this.options.loop || this.looptimes < this.options.loop))
						this.slideshowTimeout.setInterval(this.elemArray[this.curElem][16] ? this.elemArray[this.curElem][16] : this.options.slideshowinterval), this.slideshowTimeout.start(), this.isFullscreen ? b(".html5gallery-fullscreen-timer-" +
					...
					(this.elemArray[this.curElem][5] || (this.options.lightbox || this.elemArray[this.curElem][22]) && !this.isFullscreen) && (!this.options.linkonlyonvideo || !(5 == a || 6 == a || 7 == a || 8 == a)) ? (c.css({
						...
						(g.options.lightbox || g.elemArray[g.curElem][22]) &&
						!g.isFullscreen ? g.goFullscreen() : g.elemArray[g.curElem][5] && (g.elemArray[g.curElem][6] ? window.open(g.elemArray[g.curElem][5], g.elemArray[g.curElem][6]) : window.open(g.elemArray[g.curElem][5]))
					})) : (c.css({
						cursor: ""
					}), c.off("click"))
				}
			...
			showImage: function() {
				...
				b(d).on("load", function() {
					...
					if (c.isFullscreen) {
						...
						d = "fill" == c.options.resizemode ? Math.max(c.options.width / this.width,
						c.options.height / this.height) : Math.min(c.options.width / this.width, c.options.height / this.height);
					e = Math.round(d * this.width);
					d = Math.round(d * this.height);
					...
					e = b("<div class='html5gallery-elem-img-" + c.id + "' style='display:block; position:absolute; overflow:hidden; width:" + h + "px; height:" + l + "px; left:0px; margin-left:" + (c.options.slideshadow &&
			...
			showPoster: function() {
				...
				b(h).on("load", function() {
					...
					if (!b(".html5gallery-video-play-" + d.id, a).length)
						b("<div class='html5gallery-video-play-" + d.id + "' style='position:absolute;display:block;cursor:pointer;top:50%;left:50%;width:64px;height:64px;margin-left:-32px;margin-top:-32px;background:url(\"" + d.options.skinfolder + "playvideo_64.png\") no-repeat center center;'></div>").appendTo(c).off(d.eClick).on(d.eClick,
						function() {
			...
			playVideo: function() {
				...
				if (5 == a || 6 == a || 7 == a || 8 == a)
					this.isHTML5 && 0 < b("#html5gallery-elem-html5-video-" + this.id).length ? b("#html5gallery-elem-html5-video-" + this.id, this.container).get(0).play() : (a = this.getFlashMovieObject("html5gallery-elem-video-flash-" + this.id)) && "function" === typeof a.playVideo && a.playVideo()
			},
			pauseVideo: function() {
				var a = this.elemArray[this.curElem][9];
				if (5 == a || 6 == a || 7 == a || 8 == a)
					this.isHTML5 && 0 < b("#html5gallery-elem-html5-video-" + this.id).length ? b("#html5gallery-elem-html5-video-" + this.id, this.container).get(0).pause() : (a = this.getFlashMovieObject("html5gallery-elem-video-flash-" + this.id)) && "function" === typeof a.pauseVideo && a.pauseVideo()
			},
			showVideo: function(a, c) {
				...
				if (this.isFullscreen) {
					...
					b(".html5gallery-elem-" + this.id, this.$gallery).css({
				...
				if (this.options.isIE678 || this.options.isIE9 && this.options.useflashonie9 || this.options.isIE10 && this.options.useflashonie10 || this.options.isIE11 && this.options.useflashonie11)
					...
					this.options.isOpera) && (this.elemArray[this.curElem][3] || this.elemArray[this.curElem][4]))
						this.isHTML5 = !0;
				if (this.isHTML5) {
					var l = this.elemArray[this.curElem][2],
						g = this.elemArray[this.curElem][13];
					if (this.options.isFirefox || this.options.isOpera || !l)
						l = this.elemArray[this.curElem][4] ? this.elemArray[this.curElem][4] : this.elemArray[this.curElem][3], g = this.elemArray[this.curElem][15] ? this.elemArray[this.curElem][15] : this.elemArray[this.curElem][14];
					this.embedHTML5Video(b(".html5gallery-elem-video-" + this.id,
					d), f, e, l, g, a, h, c)
				} else
					h = this.elemArray[this.curElem][2], "/" != h.charAt(0) && ("http:" != h.substring(0, 5) && "https:" != h.substring(0, 6)) && (h = this.options.htmlfolder + h), l = "", this.elemArray[this.curElem][13] && (l = this.elemArray[this.curElem][13], "/" != l.charAt(0) && ("http:" != l.substring(0, 5) && "https:" != l.substring(0, 6)) && (l = this.options.htmlfolder + l)), f = {
						width: f,
						height: e,
			...
			showSWF: function() {
				...
				b(".html5gallery-elem-" +
				this.id, a).html("<div class='html5gallery-elem-flash-" + this.id + "' style='display:block;position:absolute;overflow:hidden;top:" + e + "px;left:" + f + "px;width:" + c + "px;height:" + d + "px;'></div>" + this.options.watermarkcode);
			...
			prepareYoutubeHref: function(a) {
				var b =
				"https://www.youtube.com/embed/" + a.match(/(\?v=|\/\d\/|\/embed\/|\/v\/|\.be\/)([a-zA-Z0-9\-\_]+)/)[2];
			...
			initYoutubeApi: function() {
				...
				for (a = 0; a < this.elemArray.length; a++)
					9 ==
					this.elemArray[a][9] ? b = !0 : 10 == this.elemArray[a][9] && (d = !0);
			...
			showYoutube: function(a) {
				...
				if (ASYouTubeIframeAPIReady && !this.options.isMobile && !this.options.isIE6 && !this.options.isIE7) {
					...
					f = this.prepareYoutubeHref(f), a && (f =
			...
			onThumbOver: function(a) {
				if (this.options.onthumbover && window[this.options.onthumbover] && "function" == typeof window[this.options.onthumbover])
					window[this.options.onthumbover](this.elemArray[a])
			},
			...
			embedFlash: function(a, b, d, e, f, h, k) {
				if (this.options.flashInstalled) {
					...
					window.HTML5GallerySWFObject.embedSWF(e, k, b, d, "9.0.0",
					!1, h, g, {})
				} else
					a.html("<div class='html5gallery-elem-error-" + this.id + "' style='display:block; position:absolute; text-align:center; width:" + this.options.width + "px; left:0px; top:" + Math.round(this.options.height / 2 - 10) + "px;'><div class='html5gallery-error-" + this.id + "'><div>The required Adobe Flash Player plugin is not installed</div><div style='display:block;position:relative;text-align:center;width:112px;height:33px;margin:0px auto;'><a href='http://www.adobe.com/go/getflashplayer'><img src='http://www.adobe.com/images/shared/download_buttons/get_flash_player.gif' alt='Get Adobe Flash player' width='112' height='33'></img></a></div></div>")
			}
		};
		this.each(function() {
			var a = b(this);
			k = k || {};
			for (var c in k)
				c.toLowerCase() !== c && (k[c.toLowerCase()] = k[c], delete k[c]);
			if ((!b(this).data("donotinit") || k && k.forceinit) && !b(this).data("inited")) {
				...
				var j = {
					...
					s = "vertical" == this.options.skin ? b.extend(s, m) : "showcase" == this.options.skin ? b.extend(s, n) : "light" == this.options.skin ? b.extend(s, j) : "gallery" == this.options.skin ? b.extend(s, h) : "gallerywithtext" == this.options.skin ? b.extend(s, l) : "horizontal" == this.options.skin ? b.extend(s, c) : "darkness" == this.options.skin ? b.extend(s, g) : "mediapage" == this.options.skin ? b.extend(s, f) : b.extend(s, c);
				this.options = b.extend(s,
				this.options);
				...
				j = f.length;
				for (c = 0; 5 > c; c++)
					f = f.slice(0, j - 9 + c) + f.slice(j - 8 + c);
				-1 != this.options.htmlfolder.indexOf(f) && (this.options.fv = !1);
				j = b(window).width();
				if (this.options.screenquery)
					for (c in this.options.screenquery)
						j <= this.options.screenquery[c].screenwidth && (this.options.screenquery[c].gallerywidth && (this.options.width = this.options.screenquery[c].gallerywidth), this.options.screenquery[c].galleryheight && (this.options.height = this.options.screenquery[c].galleryheight), this.options.screenquery[c].thumbwidth &&
						(this.options.thumbwidth = this.options.screenquery[c].thumbwidth), this.options.screenquery[c].thumbheight && (
							// this.options.thumbheight = this.options.screenquery[c].thumbheight
							true
							));
...
function HTML5GalleryTimer(u, v, n) {
	...
	this.pause = function() {
		m && (z = !0, clearInterval(x))
	};
	this.resume = function(u) {
		if (!b || u)
			b = !1, m && z && (z = !1, x = setInterval(function() {
				k += 50;
...
function HTML5GallerySWFObjectFunc() {
	...
	function n(a) {
		if (typeof t.addEventListener != f)
			t.addEventListener("load", a, !1);
		else if (typeof q.addEventListener != f)
		...
		if (0 < b)
			for (var c = 0; c < b; c++) {
				...
				if (0 < r.pv[0]) {
					var l = e(g);
					if (l)
						if (a(H[c].swfVersion) && !(r.wk && 312 > r.wk))
							d(g, !0), h && (j.success = !0, j.ref = x(g), h(j));
						else if (H[c].expressInstall && k()) {
							j = {};
							j.data = H[c].expressInstall;
							...
							for (var p = {}, l = l.getElementsByTagName("param"), n = l.length, q = 0; q < n; q++)
								"movie" != l[q].getAttribute("name").toLowerCase() && (p[l[q].getAttribute("name")] = l[q].getAttribute("value"));
							z(j, p, g, h)
						} else
							m(l), h && h(j)
				} else if (d(g, !0), h) {
					if ((g = x(g)) && typeof g.SetVariable != f)
						j.success = !0, j.ref = g;
					h(j)
				}
			}
	...
	function x(a) {
		var b = null;
		if ((a = e(a)) && "OBJECT" == a.nodeName)
			typeof a.SetVariable != f ? b = a : (a = a.getElementsByTagName(h)[0]) && (b = a);
		return b
	}
	function k() {
		return !I &&
	...
	function z(a, c, d, h) {
		...
		var j = e(d);
		if (j) {
			"OBJECT" == j.nodeName ? (P = b(j), R = null) : (P = j, R = d);
			a.id = g;
			if (typeof a.width == f || !/%$/.test(a.width) && 310 > parseInt(a.width, 10))
				a.width = "310";
			if (typeof a.height == f || !/%$/.test(a.height) && 137 > parseInt(a.height, 10))
				a.height = "137";
	...
	function m(a) {
		if (r.ie && r.win && 4 != a.readyState) {
			...
			a.parentNode.replaceChild(b(a), a)
	}
	function b(a) {
		...
		if (g)
			if (typeof a.id == f && (a.id = c), r.ie && r.win) {
				var j =
					...
					k;
				for (k in a)
					a[k] != Object.prototype[k] && ("data" == k.toLowerCase() ? b.movie = a[k] : "styleclass" == k.toLowerCase() ? j += ' class="' + a[k] + '"' : "classid" != k.toLowerCase() && (j += " " + k + '="' + a[k] + '"'));
				k = "";
				for (var m in b)
					b[m] != Object.prototype[m] && (k += '<param name="' + m + '" value="' + b[m] + '" />');
				...
				for (var n in a)
					a[n] != Object.prototype[n] && ("styleclass" ==
					n.toLowerCase() ? m.setAttribute("class", a[n]) : "classid" != n.toLowerCase() && m.setAttribute(n, a[n]));
				for (j in b)
					b[j] != Object.prototype[j] && "movie" != j.toLowerCase() && (a = m, k = j, n = b[j], c = q.createElement("param"), c.setAttribute("name", k), c.setAttribute("value", n), a.appendChild(c));
	...
	function c(a, b, c, d) {
		if (!r.ie || !r.mac) {
			...
			if (e) {
				...
				if (!B ||
				O != c)
					d = q.createElement("style"), d.setAttribute("type", "text/css"), d.setAttribute("media", c), B = e.appendChild(d), r.ie && (r.win && typeof q.styleSheets != f && 0 < q.styleSheets.length) && (B = q.styleSheets[q.styleSheets.length - 1]), O = c;
	...
	var f = "undefined",
		...
		C = !0,
		r,
		S = typeof q.getElementById != f && typeof q.getElementsByTagName != f && typeof q.createElement != f,
		J = G.userAgent.toLowerCase(),
		E = G.platform.toLowerCase(),
		M = E ? /win/.test(E) : /win/.test(J),
		E = E ? /mac/.test(E) : /mac/.test(J),
		J = /webkit/.test(J) ? parseFloat(J.replace(/^.*webkit\/(\d+(\.\d+)?).*$/, "$1")) : !1,
		K = !+"\v1",
		F = [0, 0, 0],
		y = null;
	if (typeof G.plugins != f && typeof G.plugins["Shockwave Flash"] ==
	h) {
		if ((y = G.plugins["Shockwave Flash"].description) && !(typeof G.mimeTypes != f && G.mimeTypes[l] && !G.mimeTypes[l].enabledPlugin))
			V = !0, K = !1, y = y.replace(/^.*\s+(\S+\s+\S+$)/, "$1"), F[0] = parseInt(y.replace(/^(.*)\..*$/, "$1"), 10), F[1] = parseInt(y.replace(/^.*\.(.*)\s.*$/, "$1"), 10), F[2] = /[a-zA-Z]/.test(y) ? parseInt(y.replace(/^.*[a-zA-Z]+(.*)$/, "$1"), 10) : 0
	} else if (typeof t.ActiveXObject != f)
		try {
			var L = new ActiveXObject("ShockwaveFlash.ShockwaveFlash");
			if (L && (y = L.GetVariable("$version")))
				K = !0, y = y.split(" ")[1].split(","),
	...
	r.ie && r.win && window.attachEvent("onunload", function() {
		...
		if (window.AmazingSWFObject) {
			for (var d in window.AmazingSWFObject)
				window.AmazingSWFObject[d] = null;
	...
	return {
		...
		embedSWF: function(b, c, e, g, j, l, m, n, p, q) {
			...
			r.w3 && !(r.wk && 312 > r.wk) && b && c && e && g && j ? (d(c, !1), v(function() {
				...
				if (m && typeof m === h)
					for (var v in m)
						t.flashvars = typeof t.flashvars != f ? t.flashvars +
						("&" + v + "=" + m[v]) : v + "=" + m[v];
				if (a(j))
					u = D(r, t, c), r.id == c && d(c, !0), s.success = !0, s.ref = u;
				else {
					if (l && k()) {
						r.data = l;
						z(r, t, c, q);
			...
			if (b) {
				...
				for (var b = b.split("&"), c = 0; c < b.length; c++)
					if (b[c].substring(0, b[c].indexOf("=")) == a)
						return j(b[c].substring(b[c].indexOf("=") + 1))
			}
			return ""
		},
		expressInstallCallback: function() {
			if (I) {
				var a = e(g);
				a && P && (a.parentNode.replaceChild(P, a), R && (d(R, !0), r.ie && r.win && (P.style.display = "block")), w && w(W));
				I = !1
			}
		}
	}
