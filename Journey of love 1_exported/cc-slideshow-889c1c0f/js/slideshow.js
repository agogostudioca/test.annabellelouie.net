(function() {
  ...
  $rs(document).ready(function($rs) {
    var mySlider, options, regex
      ...
      , arr_of_id = $rs("#cc-slideshow-889c1c0f .slides_container > [id]").map(function(){
    ...
    $rs("#cc-slideshow-889c1c0f .slide." + app.configSlideshow().active_slide_class).css('opacity', 1);
    options = {
      container: $rs('#cc-slideshow-889c1c0f .slides_container').get(0),
      slides: $rs('#cc-slideshow-889c1c0f .slide').get(),
      autoPlay: $rs("#cc-slideshow-889c1c0f .slideshow").data('autoplay') || false,
      rotationSpeed: $rs("#cc-slideshow-889c1c0f .slideshow").data('rotationspeed') || "3500",
      loop: false,
      iterations: $rs("#cc-slideshow-889c1c0f .slideshow").data('iterations') || Infinity,
      endslide: $rs("#cc-slideshow-889c1c0f .slideshow").data('endslide'),
      transition: {
        name: $rs("#cc-slideshow-889c1c0f .slideshow").data('transition') || "default"
      }
    };
    ...
    if ($rs("#cc-slideshow-889c1c0f .slideshow").data('initial-transition') === true) {
      mySlider.currentSlide = 1;

      mySlider.goTo(0, {name: $rs("#cc-slideshow-889c1c0f .slideshow").data('transition')});
    ...
    if (!mySlider.status.playing) {
      $rs("#cc-slideshow-889c1c0f .icon-pause").attr("class", "icon-play-3");
    }

    $rs("#cc-slideshow-889c1c0f .pause, #cc-slideshow-889c1c0f .play").click(function(e) {
      var $icon;
      e.preventDefault();
      $icon = $rs(this).find('i');
    ...
    $rs('#cc-slideshow-889c1c0f [class*="coffee-slider-"]').click(function(e) {
      var command;
      e.preventDefault();
    ...
    mySlider.on("transitionComplete", function() {
      var slides_indicator = $rs('#cc-slideshow-889c1c0f .slides_indicator')
        , index =$rs('#cc-slideshow-889c1c0f .slide.' + app.configSlideshow().active_slide_class).index();
    ...
    $rs("a").click(function(e) {
      var id = this.href.match(/#(?:(?!\?).)*/g)
        , index;
      if ($rs(this).is('#cc-slideshow-889c1c0f .slide_node')) { return goToFromIndicator(e); }
    ...
    swipedetect($rs("#cc-slideshow-889c1c0f .slides_container")[0], function(direction) {
      switch(direction) {
        case "right":
