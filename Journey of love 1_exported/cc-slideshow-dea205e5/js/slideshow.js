(function() {
  ...
  $rs(document).ready(function($rs) {
    var mySlider, options, regex
      ...
      , arr_of_id = $rs("#cc-slideshow-dea205e5 .slides_container > [id]").map(function(){
    ...
    $rs("#cc-slideshow-dea205e5 .slide." + app.configSlideshow().active_slide_class).css('opacity', 1);
    options = {
      container: $rs('#cc-slideshow-dea205e5 .slides_container').get(0),
      slides: $rs('#cc-slideshow-dea205e5 .slide').get(),
      autoPlay: $rs("#cc-slideshow-dea205e5 .slideshow").data('autoplay') || false,
      rotationSpeed: $rs("#cc-slideshow-dea205e5 .slideshow").data('rotationspeed') || "3500",
      loop: false,
      iterations: $rs("#cc-slideshow-dea205e5 .slideshow").data('iterations') || Infinity,
      endslide: $rs("#cc-slideshow-dea205e5 .slideshow").data('endslide'),
      transition: {
        name: $rs("#cc-slideshow-dea205e5 .slideshow").data('transition') || "default"
      }
    };
    ...
    if ($rs("#cc-slideshow-dea205e5 .slideshow").data('initial-transition') === true) {
      mySlider.currentSlide = 1;

      mySlider.goTo(0, {name: $rs("#cc-slideshow-dea205e5 .slideshow").data('transition')});
    ...
    if (!mySlider.status.playing) {
      $rs("#cc-slideshow-dea205e5 .icon-pause").attr("class", "icon-play-3");
    }

    $rs("#cc-slideshow-dea205e5 .pause, #cc-slideshow-dea205e5 .play").click(function(e) {
      var $icon;
      e.preventDefault();
      $icon = $rs(this).find('i');
    ...
    $rs('#cc-slideshow-dea205e5 [class*="coffee-slider-"]').click(function(e) {
      var command;
      e.preventDefault();
    ...
    mySlider.on("transitionComplete", function() {
      var slides_indicator = $rs('#cc-slideshow-dea205e5 .slides_indicator')
        , index =$rs('#cc-slideshow-dea205e5 .slide.' + app.configSlideshow().active_slide_class).index();
    ...
    $rs("a").click(function(e) {
      var id = this.href.match(/#(?:(?!\?).)*/g)
        , index;
      if ($rs(this).is('#cc-slideshow-dea205e5 .slide_node')) { return goToFromIndicator(e); }
    ...
    swipedetect($rs("#cc-slideshow-dea205e5 .slides_container")[0], function(direction) {
      switch(direction) {
        case "right":
